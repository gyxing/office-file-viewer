...
Word、Excel 以及 PPT/PPTX/PPTM/POTX 格式中的独立内容图片默认支持双击、`Enter` 或空格键打开预览层。预览层支持适应窗口、`10%` 到 `500%` 缩放、拖拽、顺时针旋转、复位和下载；图片右键菜单只提供“预览”和“下载”。PowerPoint 根据资源类型、对象元数据和显示尺寸综合识别小图标，小图标不提供图片预览入口或自定义预览菜单，母版、版式、背景和形状图片填充不属于该交互范围。
...
关闭 `contextMenu` 后保留双击和键盘预览，并恢复浏览器原生右键菜单。页眉装饰图、背景、水印、页面绘图层和图表不属于该交互范围；PowerPoint 仅对幻灯片中的独立内容图片提供该交互。
...
- Word、Excel 和 PowerPoint 格式的独立可见内容图片支持双击预览、缩放、旋转、下载和自定义右键菜单；PowerPoint 小图标不提供图片预览入口或自定义预览菜单，母版、版式、背景及形状图片填充保持静态。
